...
public class MTChemicalTwister extends MTMultiMachineBase<MTChemicalTwister>
     ...
     * exactly the art {@code MTEQuantumForceTransformer#getTexture} draws on its controller (idle and running, each
     * with its glow layer). The structure of that level is the QFT shell, so its controller looks like the QFT
     * controller.
     * <p>
    ...
    */
    ...
    public void getExtraInfoData(List<String> info) {
        ...
        // GT5U of this GTNH line has no IGregTechDeviceInformation.encode(); its own EBF formats the very same
        // translation key by hand, so the scanner, the sensor card and the metrics read the same line as there.
        info.add(
            StatCollector
                .translateToLocalFormatted("GT5U.EBF.heat.s", NumberFormatUtil.formatNumber(mHeatingCapacity)));
    }
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addMachineType(translateToLocal("machine.chemicaltwister.machinetype"))
            .addSeparator()
            .addInfo(translateToLocal("mt.machine.tooltip.twisted"))
            .addSubChannelUsage(GTStructureChannels.HEATING_COIL)
            .addStructureInfo(translateToLocal("machine.chemicaltwister.structureinfo.1"))
            .addSeparator()
            .addInfo(EnumChatFormatting.AQUA + translateToLocal("machine.largechemicaltwister.tooltip.modes"))
            .addInfo(EnumChatFormatting.GOLD + translateToLocal("machine.largechemicaltwister.tooltip.qft"))
            .addSeparator()
            .addInfo(EnumChatFormatting.YELLOW + translateToLocal("machine.chemicaltwister.poc"))
            .toolTipFinisher();
        return tt;
    }
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, ForgeDirection side, ForgeDirection facing,
        ...
        // the BulkProductionFrame. See getCasingTexture and STRUCTURE_LEVEL_TIER2.
        // This GT5U line has no BlockIcons#createTextureWithCasing(), so the layers that helper would compose are put
        // together here, the same way MTNQDAFReactor draws the face it borrowed.
        ITexture casing = getCasingTexture();
        if (side != facing) return new ITexture[] { casing };

        if (mStructureLevel >= STRUCTURE_LEVEL_TIER2) {
            return aActive ? face(casing, QFT_FACE_ACTIVE, QFT_FACE_ACTIVE_GLOW)
                : face(casing, QFT_FACE, QFT_FACE_GLOW);
        }
        return aActive
            ? face(
                casing,
            ...
            : face(casing, OVERLAY_FRONT_LARGE_CHEMICAL_REACTOR, OVERLAY_FRONT_LARGE_CHEMICAL_REACTOR_GLOW);
    }

    /** The controller face: the casing, the face art and its glow layer, in the order GT draws them. */
    private static ITexture[] face(ITexture casing, IIconContainer art, IIconContainer glow) {
