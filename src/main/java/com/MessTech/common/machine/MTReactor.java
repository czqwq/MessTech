...
public class MTReactor extends MTGeneratorMultiBase<MTReactor> implements ISurvivalConstructable {
     ...
     * They live in the gregtech assets: a name without a {@code modid:} prefix is completed with the GregTech domain by
     * the icon container (the same lookup {@code MTENeutronActivator} uses), so the bare path is enough here. The two
     * glow icons are optional, exactly as in the machine they come from. See {@link #getTexture}.
     */
    ...
    public void checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack, List<StructureError> errors) {
        ...
        // At least one dynamo hatch: GT5U of this GTNH line has no checkHasAnyDynamo(), so the check is the hatch
        // count error GT's own MTEEnhancedMultiBlockBase reports (same idiom as MTNQDAFReactor).
    ...
    protected MultiblockTooltipBuilder createTooltip() {
        ...
        tt.addMachineType(StatCollector.translateToLocal("machine.mtreactor.machinetype"))
            ...
            .addCasingInfoRange(StatCollector.translateToLocal("gt.blockcasings2.9.name"), 0, 25, false)
