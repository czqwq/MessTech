...
/**
 * Full port of the original GT5U Space Miner GUI, adapted to the wireless
 * {@link SpaceModuleMinerInfinity} module.
 */
public class SpaceModuleMinerInfinityGui extends SpaceModuleInfinityGui {

    private final SpaceModuleMinerInfinity miner;
    ...
    public SpaceModuleMinerInfinityGui(SpaceModuleMinerInfinity multiblock) {
        super(multiblock);
        this.miner = multiblock;
    }

    @Override
    protected ListWidget<IWidget, ?> createTerminalTextWidget(PanelSyncManager syncManager, ModularPanel parent) {
        IntSyncValue distanceSyncer = syncManager.findSyncHandler("distance", IntSyncValue.class);
        IntSyncValue cycleSyncer = syncManager.findSyncHandler("cycle", IntSyncValue.class);
        ListWidget<IWidget, ?> minerInfo = new ListWidget<>().child(
            IKey.dynamic(
                () -> EnumChatFormatting.WHITE + StatCollector.translateToLocal("tt.spaceminer.textFieldDistance")
                    ...
                    + (cycleSyncer.getValue() == 1 ? miner.getCycleDistance() : distanceSyncer.getValue()))
                .asWidget()
                .marginBottom(2)
                .leftRel(0));
    ...
    protected Flow createLeftPanelGapRow(ModularPanel parent, PanelSyncManager syncManager) {
        ...
        row.child(createFilterButton());
        row.child(createCalculatorButton());
        row.child(createSpaceMinerUtilityButton(parent, syncManager));
        return row;
    }
    ...
    protected Flow createRightPanelGapRow(ModularPanel parent, PanelSyncManager syncManager) {
        ...
        IPanelHandler editParametersPanel = syncManager.syncedPanel(
            "editParameters",
            true,
        ...
        row.child(
            new ButtonWidget<>().size(18, 18)
                ...
                .tooltip(t -> t.addLine(IKey.lang("machine.spacemoduleminer.editParameters")))
    ...
    private ModularPanel openEditParametersPanel(ModularPanel parent, PanelSyncManager syncManager) {
        IntSyncValue distanceSync = syncManager.findSyncHandler("distance", IntSyncValue.class);
        IntSyncValue overdriveSync = syncManager.findSyncHandler("overdrive", IntSyncValue.class);
        IntSyncValue cycleSync = syncManager.findSyncHandler("cycle", IntSyncValue.class);
        IntSyncValue rangeSync = syncManager.findSyncHandler("range", IntSyncValue.class);
        IntSyncValue stepSync = syncManager.findSyncHandler("step", IntSyncValue.class);

        ModularPanel panel = new ModularPanel("editParameters").size(110, 190)
            .relative(parent)
            .leftRel(1)
            .topRel(0.7f);
        ...
        addEditField(column, "machine.spacemoduleminer.distance", distanceSync, 0, 300, 30);
        addEditField(column, "machine.spacemoduleminer.overdrive", overdriveSync, 100, 200, 100);
        addEditField(column, "machine.spacemoduleminer.cycle", cycleSync, 0, 1, 0);
        addEditField(column, "machine.spacemoduleminer.range", rangeSync, 0, 150, 0);
        addEditField(column, "machine.spacemoduleminer.step", stepSync, 0, 20, 1);

        panel.child(column);
        return panel;
    }

    private void addEditField(Flow column, String langKey, IntSyncValue syncer, int min, int max, int def) {
        column.child(
            IKey.lang(langKey)
                .asWidget()
                .marginTop(3));
    ...
    private IWidget createFilterButton() {
        ...
        return new ButtonWidget<>().overlay(new DynamicDrawable(() -> {
            if (miner.isWhitelisted) return GTGuiTextures.TT_OVERLAY_BUTTON_WHITELIST;
    ...
    private IWidget createFilterSlotGroup(PanelSyncManager syncManager) {
        ...
        if (miner.wasFilterPasted) {
            miner.wasFilterPasted = false;
            filterSyncer.updateCacheFromSource(false);
        }

        for (int i = 0; i < 64; i++) {
            filterModularSlots[i] = new ModularSlot(miner.filterInventory, i) {

                @Override
                public void onSlotChanged() {
                    miner.generateOreConfigurationList();
                }
            };
    ...
    private boolean currentOresContainThis(ItemStack stack, int distance, int droneTier, PanelSyncManager syncManager,
        ...
        IntSyncValue rangeSyncer = syncManager.findSyncHandler("range", IntSyncValue.class);
        IntSyncValue cycleSyncer = syncManager.findSyncHandler("cycle", IntSyncValue.class);

        int range = cycleSyncer.getValue() == 1 ? rangeSyncer.getValue() : 0;
        int start = Math.max(1, distance - range);
        int end = Math.min(SpaceModuleMinerInfinity.MAX_DISTANCE, distance + range);

        if (checked[index] && droneCache == droneTier
            && cycleCache == (cycleSyncer.getValue() == 1)
            && startCache == start
            && endCache == end) {
            ...
            List<Pair<Integer, GTRecipe>> asteroidsAtDistance = asteroids.get(i)
                ...
                .filter(
                    asteroid -> asteroid.second()
                        .getMetadata(IGRecipeMaps.MODULE_TIER) <= miner.getModuleTier())
    ...
    private IWidget createFilterPanelConfiguration(PanelSyncManager syncManager, ModularPanel panel) {
        IntSyncValue distanceParameterSyncer = syncManager.findSyncHandler("distance", IntSyncValue.class);
        ...
        return new DynamicDrawable(() -> {
            if (filterContainsAsteroidOre(data)) {
                return new DrawableStack(
                    ...
                    new Rectangle().color(miner.isWhitelisted ? Color.rgb(0, 255, 0) : Color.rgb(255, 0, 0))
                        .asIcon()
                        .size(16));
            } else {
    ...
    private IGuiAction.MousePressed createDropDisplayOnMousePressed(ItemStack ore) {
        return mouseData -> {
            ...
            if (firstEmptySlot < 0) return true;
    ...
    private boolean filterContainsOre(ItemStack ore) {
        for (int i = 0; i < 64; i++) {
            if (miner.filterInventory.getStackInSlot(i) != null && miner.filterInventory.getStackInSlot(i)
                .getDisplayName()
    ...
    private int findFirstEmptySlot() {
        for (int i = 0; i < 64; i++) {
            if (miner.filterInventory.getStackInSlot(i) == null) {
                return i;
            }
        }
        return -1;
    ...
    private int findFirstEmptySlot(List<Integer> visited) {
        for (int i = 0; i < 64; i++) {
            if (miner.filterInventory.getStackInSlot(i) == null && !visited.contains(i)) {
                return i;
            }
        }
        return -1;
    ...
    private Integer optimizeDistance(int asteroidIndex, int droneTier) {
        ...
        for (Map.Entry<Integer, List<Pair<Integer, GTRecipe>>> asteroidList : asteroids.entrySet()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (recipe.getMetadata(IGRecipeMaps.MODULE_TIER) > miner.getModuleTier()) continue;
        ...
        return droneTier < 0 ? new ArrayList<>()
            : SpaceMiningRecipes.asteroidDistanceMap.get(droneTier)
                ...
                .filter(pair -> {
                    ...
                    return requiredModuleTier != null && moduleTier >= requiredModuleTier;
                })
                .sorted(
    ...
    protected void registerSyncValues(PanelSyncManager syncManager) {
        ...
        syncManager.syncValue("distance", new IntSyncValue(miner::getDistance, miner::setDistance).allowC2S());
        syncManager.syncValue("overdrive", new IntSyncValue(miner::getOverdrive, miner::setOverdrive).allowC2S());
        syncManager
            .syncValue("cycle", new IntSyncValue(miner::getCycleEnabledInt, miner::setCycleEnabledInt).allowC2S());
        syncManager.syncValue("range", new IntSyncValue(miner::getRange, miner::setRange).allowC2S());
        syncManager.syncValue("step", new IntSyncValue(miner::getStep, miner::setStep).allowC2S());
        syncManager
            .syncValue("whitelist", new IntSyncValue(miner::getWhitelistedInt, miner::setWhitelistedInt).allowC2S());

        GenericSyncValue<ItemStackHandler, ?> filterSyncer = new GenericSyncValue<>(
            ItemStackHandler.class,
            () -> miner.filterInventory,
            handler -> miner.filterInventory = handler,
        ...
        BooleanSyncValue isWhiteListedSyncer = new BooleanSyncValue(
            () -> miner.isWhitelisted,
            val -> miner.isWhitelisted = val).allowC2S();
        syncManager.syncValue("isWhiteListed", isWhiteListedSyncer);

        IntSyncValue droneTierSyncer = new IntSyncValue(
            () -> miner.currentDroneMask <= 0 ? -1
                : (int) Math.round((Math.log(miner.currentDroneMask) / Math.log(2))));
    ...
    private static class ItemStackListAdapter implements IByteBufAdapter<ItemStackHandler> {
        ...
        @Override
        public void serialize(PacketBuffer buffer, ItemStackHandler value) throws IOException {
            buffer.writeNBTTagCompoundToBuffer(value.serializeNBT());
        }
